...
export class UserAjaxService {
    ...
    getPageByLoansNumberDesc(size: number | undefined, page: number | undefined): Observable<IUserPage> {
        if (!size) size = 10;
        if (!page) page = 0;
        return this.oHttpClient.get<IUserPage>(this.sUrl + "/byLoansNumberDesc?size=" + size + "&page=" + page);
    }
